...
export function PRDiffStudio({ apiKey, onReviewCreated, selectedReview }: PRDiffStudioProps) {
  ...
  return (
    <div className="p-6 sm:p-10 space-y-8 max-w-[1600px] mx-auto bg-[#FDFBFC] text-[#201E1E] font-sans">
      {/* Top Controls Bar */}
      <div className="bg-[#F4EFEB] border border-[#A68B78]/30 p-6 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <form onSubmit={handleRunAudit} className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-mono text-[#634F43] uppercase mb-1.5 font-medium">Target PR URL</label>
            <input
              type="text"
              value={prUrl}
              onChange={(e) => setPrUrl(e.target.value)}
              className="w-full bg-[#FDFBFC] border border-[#A68B78]/30 px-3.5 py-2.5 text-xs font-mono text-[#201E1E] focus:outline-none focus:border-[#164A40]"
              placeholder="https://github.com/owner/repo/pull/1"
            />
          </div>

          <div>
            <label className="block text-xs font-mono text-[#634F43] uppercase mb-1.5 font-medium">Commit SHA</label>
            <input
              type="text"
              value={headCommitSha}
          ...
          <div className="flex items-end">
            <button
              ...
              className="w-full py-3 px-5 bg-[#164A40] hover:bg-[#0f362e] text-[#FDFBFC] hover:text-[#F7D3CC] font-sans font-semibold text-xs tracking-wide transition-colors flex items-center justify-center space-x-2 shadow-sm"
            >
              {isAnalyzing ? (
                <>
        ...
        <div className="flex items-center space-x-8 border-l border-[#A68B78]/25 pl-8 font-sans">
          <div>
            <span className="text-[#634F43] block text-[10px] uppercase font-mono tracking-wider">Audit Score</span>
            <span className="text-2xl font-bold font-mono text-[#164A40]">{currentReview.overallScore} <span className="text-xs font-sans font-normal text-[#634F43]">/ 100</span></span>
          </div>
          <div>
            <span className="text-[#634F43] block text-[10px] uppercase font-mono tracking-wider">Total Issues</span>
            <span className="text-2xl font-bold font-mono text-[#93000a]">{currentReview.findings?.length || 0}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-[650px]">
        ...
        <div className="lg:col-span-7 bg-[#FDFBFC] border border-[#A68B78]/30 flex flex-col shadow-sm">
          <div className="bg-[#F4EFEB] border-b border-[#A68B78]/25 px-5 py-3.5 flex items-center justify-between font-sans text-xs">
            <div className="flex items-center space-x-2 text-[#164A40]">
              ...
              <span className="text-[#634F43] font-mono">({currentReview.repository})</span>
            </div>
          ...
          <div className="flex-1 bg-[#FDFBFC] p-5 overflow-x-auto font-mono text-xs space-y-1">
            ...
            <div className="mt-6 p-5 bg-[#F4EFEB] border border-[#A68B78]/30 text-[#201E1E] space-y-2 font-sans">
              <div className="flex items-center space-x-2 text-[#164A40] font-bold">
                <Sparkles className="w-4 h-4 text-[#A68B78]" />
                <span className="font-editorial text-sm italic text-[#164A40]">AI Inline Audit Commentary</span>
              </div>
              <p className="text-xs leading-relaxed text-[#634F43]">
                Refactoring from concrete class <code className="font-mono text-[#164A40] font-bold">PgVectorEmbeddingStore</code> to interface <code className="font-mono text-[#164A40] font-bold">EmbeddingStore&lt;TextSegment&gt;</code> prevents dependency injection crashes during cloud vector DB reconnect cycles.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 bg-[#FDFBFC] border border-[#A68B78]/30 flex flex-col shadow-sm">
          <div className="bg-[#F4EFEB] border-b border-[#A68B78]/25 px-5 py-3 flex items-center space-x-6 text-xs font-sans">
            <button
              onClick={() => setActiveTab('diff')}
              className={`pb-1 font-semibold transition-colors ${activeTab === 'diff' ? 'text-[#164A40] border-b-2 border-[#164A40] font-bold' : 'text-[#634F43]'}`}
            >
              Findings ({currentReview.findings?.length || 0})
            </button>
            <button
              onClick={() => setActiveTab('fix-preview')}
              className={`pb-1 font-semibold transition-colors ${activeTab === 'fix-preview' ? 'text-[#164A40] border-b-2 border-[#164A40] font-bold' : 'text-[#634F43]'}`}
            >
              Auto-Fix Applicator
            </button>
          </div>
          ...
          <div className="flex-1 p-5 overflow-y-auto space-y-4 font-sans text-xs">
            {activeTab === 'diff' ? (
                ...
                <div
                  ...
                  className={`p-5 border transition-all cursor-pointer ${
                    activeFinding?.id === finding.id
                      ...
                      : 'bg-[#FDFBFC] border-[#A68B78]/25 hover:border-[#164A40]'
                  }`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className={`px-2.5 py-0.5 text-[10px] font-mono font-bold ${
                      finding.severity === 'CRITICAL'
                        ? 'bg-[#93000a] text-[#FDFBFC]'
                        : finding.severity === 'WARNING'
                        ? 'bg-[#164A40] text-[#F7D3CC]'
                        : 'bg-[#F4EFEB] border border-[#A68B78]/40 text-[#634F43]'
                    }`}>
                      {finding.severity}
                    </span>
                    <span className="text-[10px] font-mono text-[#A68B78]">{finding.category}</span>
                  </div>

                  <h4 className="font-bold text-sm text-[#164A40] mb-1">{finding.title}</h4>
                  <p className="text-[#634F43] text-xs leading-relaxed mb-3">{finding.description}</p>

                  <div className="bg-[#FDFBFC] p-3 border border-[#A68B78]/25 text-[#164A40] text-xs font-sans">
                    <span className="text-[#A68B78] block text-[10px] font-mono uppercase mb-0.5">Suggested Fix:</span>
                    {finding.suggestion}
                  </div>
                </div>
              ))
              ...
              <div className="space-y-4 font-sans">
                <div className="p-6 bg-[#F4EFEB] border border-[#A68B78]/30 space-y-4">
                  <h4 className="text-sm font-bold text-[#164A40]">Automated AST Code Fix</h4>
                  <p className="text-xs text-[#634F43]">Apply recommended batch insertion optimization to RAGService.java:</p>

                  <div className="bg-[#FDFBFC] p-4 border border-[#A68B78]/30 text-[#164A40] text-xs font-mono font-medium overflow-x-auto">
                    <code>
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-[#164A40] hover:bg-[#0f362e] text-[#FDFBFC] hover:text-[#F7D3CC] font-sans font-semibold text-xs transition-colors flex items-center justify-center space-x-2 shadow-sm"
                  >
